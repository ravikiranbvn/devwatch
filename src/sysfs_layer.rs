...
fn device_numbers_from_devnode(dev_path: &Path) -> Option<(u64, u64, DevNodeType)> {
    ...
    Some((major, minor, node_type))
}

fn find_sysfs_path_by_devnums(major: u64, minor: u64, node_type: DevNodeType) -> Option<PathBuf> {
    let devnum = format!("{major}:{minor}");

    let link_path = match node_type {
        DevNodeType::Char => Path::new("/sys/dev/char").join(&devnum),
        DevNodeType::Block => Path::new("/sys/dev/block").join(&devnum),
    };
...
pub fn resolve_sysfs_info(device: &DeviceUsage) -> SysfsInfo {
    let Some((major, minor, node_type)) = device_numbers_from_devnode(&device.device_path) else {
        return SysfsInfo {
            sysfs_path: None,
            subsystem: None,
            dev_numbers: None,
    ...
    let Some(sysfs_path) = find_sysfs_path_by_devnums(major, minor, node_type) else {
